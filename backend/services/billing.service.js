...
export const getInvoices = async ({ status, patientId, from, to, page = 1, pageSize = 10 }) => {
  const conditions = [];
  const params = [];
...
export const getAdminStats = async () => {
    ...
    const [todayRevenueRows] = await pool.query(
        `SELECT SUM(amount) as total FROM Payment WHERE DATE(paidAt) = CURDATE()`
    );
    
    // Tổng số tiền chưa thanh toán (từ hóa đơn UNPAID)
    const [unpaidRows] = await pool.query(
        `
        SELECT SUM(ii.quantity * ii.price) as total
        FROM Invoice i
    ...
    return {
        ...
        totalUnpaid: Number(unpaidRows[0].total || 0)
    };
};
